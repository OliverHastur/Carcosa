
     <?php require_once('header-index.php')?>

    <!--INÍCIO CATEGORIAS EM DESTAQUE-->
    <div class="categorias">
        <!--INÍCIO CORPO DAS CATEGORIAS EM DESTAQUE-->
        <div class="corpo-categorias">
            <!--INÍCIO LINHA DO CORPO DAS CATEGORIAS EM DESTAQUE-->
            <div class="linha">

                <div class="col-3">
                    <img src="/coqueteria-v1/assets/img/categoria-1.jpg" alt="">
                </div>

                <div class="col-3">
                    <img src="/coqueteria-v1/assets/img/categoria-2.jpg" alt="">
                </div>

                <div class="col-3">
                    <img src="/coqueteria-v1/assets/img/categoria-3.jpg" alt="">
                </div>
            </div>
            <!--FIM LINHA DO CORPO DAS CATEGORIAS EM DESTAQUE-->
        </div>
        <!--FIM CORPO DAS CATEGORIAS EM DESTAQUE-->
    </div>
    <!--FIM CATEGORIAS EM DESTAQUE-->
    <!--INÍCIO PRODUTOS EM DESTAQUE-->
    <div class="corpo-categorias">
        <h2 class="titulo">Produtos em Destaque</h2>
        <!--INÍCIO LINHA DO CORPO DAS CATEGORIAS EM DESTAQUE-->
        <div class="linha">
            <!--INÍCIO ITEM PRODUTO 1 EM DESTAQUE-->
            <div class="col-4">
                <a href="ver-produto.php" title="">
                    <img src="/coqueteria-v1/assets/img/produto-1.jpg" alt="">
                </a>
                <h4>Bolsa Modelo Canoa</h4>
                <div class="classificacao">
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                </div>
                <p>R$ 299,90</p>
            </div>
            <!--FIM ITEM PRODUTO 1 EM DESTAQUE-->
            <!--INÍCIO ITEM PRODUTO 2 EM DESTAQUE-->
            <div class="col-4">
                <a href="ver-produto.php" title="">
                    <img src="/coqueteria-v1/assets/img/produto-2.jpg" alt="">
                </a>
                <h4>Bucket Hat</h4>
                <div class="classificacao">
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                </div>
                <p>R$ 119,90</p>
            </div>
            <!--FIM ITEM PRODUTO 2 EM DESTAQUE-->
            <!--INÍCIO ITEM PRODUTO 3 EM DESTAQUE-->
            <div class="col-4">
                <a href="ver-produto.php" title="">
                    <img src="/coqueteria-v1/assets/img/produto-3.jpg" alt="">
                </a>
                <h4>Amigurumi</h4>
                <div class="classificacao">
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                </div>
                <p>R$ 149,90</p>
            </div>
            <!--FIM ITEM PRODUTO 3 EM DESTAQUE-->
            <!--INÍCIO ITEM PRODUTO 4 EM DESTAQUE-->
            <div class="col-4">
                <a href="ver-produto.php" title="">
                    <img src="/coqueteria-v1/assets/img/produto-4.jpg" alt="">
                </a>
                <h4>Balaclava</h4>
                <div class="classificacao">
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                </div>
                <p>R$ 139,90</p>
            </div>
            <!--FIM ITEM PRODUTO 4 EM DESTAQUE-->
        </div>
        <!--FIM LINHA DO CORPO DAS CATEGORIAS EM DESTAQUE-->
        <h2 class="titulo">Novos Produtos</h2>
        <!--INÍCIO LINHA DO CORPO DAS NOVIDADES-->
        <div class="linha">
            <!--INÍCIO ITEM PRODUTO 5 EM DESTAQUE-->
            <div class="col-4">
                <a href="ver-produto.php" title="">
                    <img src="/coqueteria-v1/assets/img/produto-5.jpg" alt="">
                </a>
                <h4>Bolsa Modelo Rio</h4>
                <div class="classificacao">
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                </div>
                <p>R$ 149,90</p>
            </div>
            <!--FIM ITEM PRODUTO 5 EM DESTAQUE-->
            <!--INÍCIO ITEM PRODUTO 6 EM DESTAQUE-->
            <div class="col-4">
                <a href="ver-produto.php" title="">
                    <img src="/coqueteria-v1/assets/img/produto-6.jpg" alt="">
                </a>
                <h4>Boné de croche</h4>
                <div class="classificacao">
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                </div>
                <p>R$ 139,90</p>
            </div>
            <!--FIM ITEM PRODUTO 6 EM DESTAQUE-->
            <!--INÍCIO ITEM PRODUTO 7 EM DESTAQUE-->
            <div class="col-4">
                <a href="ver-produto.php" title="">
                    <img src="/coqueteria-v1/assets/img/produto-7.jpg" alt="">
                </a>
                <h4>Bandana</h4>
                <div class="classificacao">
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                </div>
                <p>R$ 89,90</p>
            </div>
            <!--FIM ITEM PRODUTO 7 EM DESTAQUE-->
            <!--INÍCIO ITEM PRODUTO 8 EM DESTAQUE-->
            <div class="col-4">
                <a href="ver-produto.php" title="">
                    <img src="/coqueteria-v1/assets/img/produto-8.jpg" alt="">
                </a>
                <h4>Centro de Mesa</h4>
                <div class="classificacao">
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                </div>
                <p>R$ 99,90</p>
            </div>
            <!--FIM ITEM PRODUTO 8 EM DESTAQUE-->
            <!--INÍCIO ITEM PRODUTO 9 EM DESTAQUE-->
            <div class="col-4">
                <a href="ver-produto.php" title="">
                    <img src="/coqueteria-v1/assets/img/produto-9.jpg" alt="">
                </a>
                <h4>Sousplat</h4>
                <div class="classificacao">
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                </div>
                <p>R$ 49,90</p>
            </div>
            <!--FIM ITEM PRODUTO 9 EM DESTAQUE-->
            <!--INÍCIO ITEM PRODUTO 10 EM DESTAQUE-->
            <div class="col-4">
                <a href="ver-produto.php" title="">
                    <img src="/coqueteria-v1/assets/img/produto-10.jpg" alt="">
                </a>
                <h4>Tapete</h4>
                <div class="classificacao">
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                </div>
                <p>R$ 99,90</p>
            </div>
            <!--FIM ITEM PRODUTO 10 EM DESTAQUE-->
            <!--INÍCIO ITEM PRODUTO 11 EM DESTAQUE-->
            <div class="col-4">
                <a href="ver-produto.php" title="">
                    <img src="/coqueteria-v1/assets/img/produto-11.jpg" alt="">
                </a>
                <h4>Porta-Celular</h4>
                <div class="classificacao">
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                </div>
                <p>R$ 49,90</p>
            </div>
            <!--FIM ITEM PRODUTO 11 EM DESTAQUE-->
            <!--INÍCIO ITEM PRODUTO 12 EM DESTAQUE-->
            <div class="col-4">
                <a href="ver-produto.php" title="">
                <img src="/coqueteria-v1/assets/img/produto-12.jpg" alt="">
            </a>
                <h4>Pochete</h4>
                <div class="classificacao">
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                    <ion-icon name="star"></ion-icon>
                </div>
                <p>R$ 119,90</p>
            </div>
            <!--FIM ITEM PRODUTO 12 EM DESTAQUE-->
        </div>
        <!--FIM LINHA DO CORPO DAS NOVIDADES-->
    </div>
    <!--FIM PRODUTOS EM DESTAQUE-->

    <!--INÍCIO OFERTAS-->
    <div class="ofertas">
        <div class="corpo-categorias">
            <div class="linha">
                <div class="col-2">
                    <img src="/coqueteria-v1/assets/img/banner-2.png" alt="" class="oferta-img">
                </div>

                <div class="col-2">
                    <p>Produto Exclusivo da Coqueteria</p>
                    <h1>Completamente personalizado</h1>
                    <small>Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci sapiente minus praesentium
                        fugiat deleniti voluptate, corrupti voluptates. Rerum tempora obcaecati quia dolorum, et ex
                        sint! Repudiandae optio voluptatem officia ex.</small>
                    <br> <br> <a href="" class="btn">Comprar Agora &#8594;</a>
                </div>
            </div>
        </div>
    </div>
    <!--FIM OFERTAS-->
    <!--INÍCIO DEPOIMENTOS-->
    <div class="depoimentos">
        <div class="corpo-categorias">
            <div class="linha">
                <!--INÍCIO ITEM 1 DEPOIMENTO-->
                <div class="col-3">
                    <ion-icon name="ribbon" class="depoimento-icone"></ion-icon>
                    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci sapiente minus praesentium
                        fugiat deleniti voluptate, corrupti voluptates. Rerum tempora obcaecati quia dolorum, et ex
                        sint! Repudiandae.</p>

                    <div class="classificacao">
                        <ion-icon name="star"></ion-icon>
                        <ion-icon name="star"></ion-icon>
                        <ion-icon name="star"></ion-icon>
                        <ion-icon name="star"></ion-icon>
                        <ion-icon name="star"></ion-icon>
                    </div>
                    <img src="/coqueteria-v1/assets/img/cliente-1.png" alt="">
                    <h3>Dayde Costa</h3>
                </div>
                <!--FIM ITEM 1 DEPOIMENTO-->
                <!--INÍCIO ITEM 2 DEPOIMENTO-->
                <div class="col-3">
                    <ion-icon name="ribbon" class="depoimento-icone"></ion-icon>
                    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolorem minima quaerat nesciunt
                        perferendis necessitatibus consectetur harum. Explicabo impedit eum totam molestias! Sunt,
                        labore.</p>

                    <div class="classificacao">
                        <ion-icon name="star"></ion-icon>
                        <ion-icon name="star"></ion-icon>
                        <ion-icon name="star"></ion-icon>
                        <ion-icon name="star"></ion-icon>
                        <ion-icon name="star"></ion-icon>
                    </div>
                    <img src="/coqueteria-v1/assets/img/cliente-2.png" alt="">
                    <h3>Déssio Pinto</h3>
                </div>
                <!--FIM ITEM 2 DEPOIMENTO-->
                <!--INÍCIO ITEM 3 DEPOIMENTO-->
                <div class="col-3">
                    <ion-icon name="ribbon" class="depoimento-icone"></ion-icon>
                    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. In iste voluptates nobis quaerat quasi
                        nesciunt. Aliquid sint reprehenderit fuga, laboriosam illum dolor error non cum doloribus ab
                        provident! Numquam, similique!.</p>

                    <div class="classificacao">
                        <ion-icon name="star"></ion-icon>
                        <ion-icon name="star"></ion-icon>
                        <ion-icon name="star"></ion-icon>
                        <ion-icon name="star"></ion-icon>
                        <ion-icon name="star"></ion-icon>
                    </div>
                    <img src="/coqueteria-v1/assets/img/cliente-3.png" alt="">
                    <h3>Paula Tejano</h3>
                </div>
                <!--FIM ITEM 3 DEPOIMENTO-->
            </div>
        </div>
    </div>
    <!--FIM DEPOIMENTOS-->

    <!--INÍCIO MARCAS PARCEIRAS-->
    <div class="marcas">
        <div class="corpo-categorias">
            <div class="linha">

                <div class="col-5">
                    <img src="/coqueteria-v1/assets/img/marca-1.png" alt="">
                </div>

                <div class="col-5">
                    <img src="/coqueteria-v1/assets/img/marca-2.png" alt="">
                </div>

                <div class="col-5">
                    <img src="/coqueteria-v1/assets/img/marca-3.png" alt="">
                </div>

                <div class="col-5">
                    <img src="/coqueteria-v1/assets/img/marca-4.png" alt="">
                </div>

                <div class="col-5">
                    <img src="/coqueteria-v1/assets/img/marca-5.png" alt="">
                </div>

            </div>
        </div>
    </div>
    <!--FIM MARCAS PARCEIRAS-->

    <?php require_once('footer.php')?>
